...
public class Updater implements Runnable {
	...
	private String headcount_address = (String) Config.getReadOnly(
	...
	private void pingHeadcount() {
		SocketIOWrapper io;
		try {
			io = new SocketIOWrapper(new Socket(
					InetAddress.getByName(headcount_address), headcount_port));
			io.write(Node.getAddress().toJSONString());
			io.close();
		} catch (IOException e) {
			DBP.printerrorln("IO Error pinging Headcount server");
			DBP.printerror(e);
		}
	}
	...
	protected void update() throws InterruptedException {
		...
		/*
		 * serial code below replaced by parallel code above Set<L3Address> temp
		 * = new HashSet<L3Address>(); for(L3Address node : every1ISee){ try {
		 * temp.addAll(Router.primitiveLookup(node,
		 * Node.getOverlayAddress()).values()); } catch (IOException e) { //
		 * TODO Auto-generated catch block DBP.printException(e); } catch
		 * (RPCException e) { // TODO Auto-generated catch block
		 * DBP.printException(e); } } every1ISee.addAll(temp); temp = null;
		 */
		pool.clear();
		q.clear();
		q.addAll(every1ISee);
		...
		/*
		 * serial code below replaced by parallel code above for(L3Address node
		 * : every1ISee){ try { if(Router.ping(node)){
		 * if(!Node.getAddressTable().contains(node)) DBP.printdemoln("Adding "
		 * + node + " to table from updater"); Node.getAddressTable().add(node);
		 * forgive(node); }else{ strike(node); } } catch (RPCException e) { //
		 * TODO determine if exception was somehow not node's fault, else strike
		 * node DBP.printException(e); } }
		 */
		...
		try {
			...
			// TODO probably shouldn't be throwing this, should change in
			// StorageController
			DBP.printerrorln("trouble deleting 3rd bucket");// e.printStackTrace();
		}

		// Ping the headcount server if the flag has been set in the default
		// config file
		if (headcount_flag)
			pingHeadcount();
			...
			for (L3Address node = q.poll(); node != null; node = q.poll()) {
				if (seek) {
					...
					try {
						...
						ref.strike(node);
						// TODO determine if exception was somehow not node's
						// fault, else strike node
						DBP.printException(e);
					}
				}
			}
